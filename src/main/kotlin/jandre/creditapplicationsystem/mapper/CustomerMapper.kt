...
object CustomerMapper {
    ...
    fun ReqDtoToEntity(reqDto: CustomerReqDto, address: Address): Customer {
        return Customer(null, reqDto.firstName!!, reqDto.lastName!!,
            reqDto.cpf!!, reqDto.email!!, reqDto.income!!, reqDto.password!!, address, mutableListOf())
    }

    fun EntityToResDto(customer: Customer): CustomerResDto {
        return CustomerResDto(customer.id!!, customer.firstName,
            customer.lastName, customer.cpf, customer.email, AddressMapper.EntityToResDto(customer.address), customer.credits)
    }

    fun patchExistingCustomerUsingReqDto(reqDto: CustomerReqDto, existingEntity: Customer): Customer {
